	...
	public function __construct(){ set_error_handler('getError');
		...
		try{
			if(DB_STATUS == true){
				...
				$this->db = new sys\Error(); # Db Status false
			}
		}catch(Exception $e){
			if($e){
	...
	public function middleware($call=NULL,$route_args=[]){
		if($call!=NULL){
			if(is_string($call)){
					...
					$middleware_status = $call( $Http = (new Http()) , $Request = (new sys\Request($route_args)), $Response=(new sys\Response));
				          ...
				          ############################### AUTHENTICATION END ####################################
						   ...
						   if(($auth_head==true && $auth_basic==true) || SHA==FALSE || $this->check_auth == FALSE ){
								...
								if(is_string($call)){ # Routes
									...
									#require_once 'Response.php';
									# $Request = (object)$route_args
									$call( new Http() , $Request = (new sys\Request($route_args)), $Response=(new sys\Response));
									#unset($_GET,$_POST);
								}
