...
class import{
	...
	public function git_package($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/package'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
		$message = "<?php\nnamespace package;\nclass ".ucfirst($fileName)."{\n\n}\n?>";
		if (file_exists($file)){
		  	$msg = "\033[0;31m".ucfirst($fileName)." ".$c_dir." already exist.\033[0m \n";
		} else {  if(!is_dir($c_dir)){ $uold = umask(0); mkdir($c_dir,0777,true); umask($uold); }
	...
	public function module($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/modules'; $module = $c_dir.'/'.strtolower($fileName);
		if(is_dir($module)){
	...
	public function git_module($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/modules'; $module = $c_dir.'/'.strtolower($fileName);
		if(is_dir($module)){
		  	$msg = "\033[0;31m".$fileName." module already exist.\033[0m \n";
		}else{ 
		  if(!is_dir($c_dir)){ $uold = umask(0); mkdir($c_dir,0777); umask($uold); }
		  if(is_dir($c_dir) && is_writable($c_dir)){
		  	# Download from server & extract
		  	# https://github.com/Soava-lab/warehouse/raw/master/modules/register.zip
			$url = $this->git_uri.$c_dir.'/'.strtolower($fileName).".zip";
			$headers = self::getHeaders($url);
			$path	 = $c_dir."/".strtolower($fileName).".zip";
			if ($headers['http_code'] === 200 and $headers['download_content_length'] < 1024*1024) {
			  if (self::download($url, $path)){
