...
if($data->type!="" && $data->dest!="" && $data->content!=""){
	$type = $data->type;
	$file = "../app/".$type."/".ucfirst($data->dest).".php";
	if(file_exists($file)) die(clean_color("\033[0;31mSorry, ".ucfirst($type)." ".ucfirst($data->dest).".php already exist. \033[0m"));
	$content= base64_decode($data->content);
	if($type == 'api'){
		if(is_dir("../app/extender/sh")) { $uold = umask(0); chmod("../app/extender/sh",0777,true); umask($uold); }else{
			$uold = umask(0); mkdir("../app/extender/sh",0777,true); umask($uold);
		}
		$file = "../app/extender/sh/".ucfirst($data->dest).".php";
	...
	}else if($type == 'module'){ $uold = umask(0); chmod("../app/modules",0777,true); umask($uold);
		
	}else{ $uold = umask(0); chmod("../app/".$type,0777,true); umask($uold);
		if(file_put_contents($file,$content)){
			echo clean_color("\033[0;32m".ucfirst($type).' '.ucfirst($data->dest).'.php has been moved to server'."\033[0m");
		}else{
			echo clean_color("\033[0;31mSorry, ".ucfirst($type)." directory permission denied. \033[0m");
