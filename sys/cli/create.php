...
require_once 'db.php';
class create{
	public function controller($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/controller'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
	...
	public function model($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/model'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
	...
	public function library($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/library'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
		$message = "<?php\n#Create your public methods here to access.\nclass ".ucfirst($fileName)."{\n\n}\n?>";
		if (file_exists($file)){
		  	$msg = "\033[0;31m".ucfirst($fileName)." ".$c_dir." already exist.\033[0m \n";
		} else { if(!is_dir($c_dir)){ $uold = umask(0); mkdir($c_dir,0777,true); umask($uold); }
	...
	public function extender($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/extender'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
	...
	public function package($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/package'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
		$message = "<?php\nnamespace package;\nclass ".ucfirst($fileName)."{\n\n}\n?>";
		if (file_exists($file)){
		  	$msg = "\033[0;31m".ucfirst($fileName)." ".$c_dir." already exist.\033[0m \n";
		} else { $uold = umask(0); chmod($c_dir,0777); umask($uold);
	...
	# Create CRUD API
	public function api($fileName){ $msg=BAD_FORMAT(); $c_dir = 'app/extender/init'; $file = $c_dir.'/'.ucfirst($fileName.'.php');
